        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="border-b border-white/6 align-top last:border-b-0 hover:bg-white/[0.02]">
              {admin ? (
                <TableCell>
                  <input type="checkbox" name="ids" value={item.id} className="mt-1 h-4 w-4 rounded border-white/12 bg-transparent" />
                </TableCell>
              ) : null}
              <TableCell>
                <Link href={`/operacao/excecoes/${item.id}`} className="font-medium text-white transition hover:text-sky-100">{item.code} · {item.title}</Link>
                <div className="mt-1 text-xs leading-5 text-slate-500">{linkLabel(item)} · prioridade {item.priorityScore}</div>
                <div className="mt-1 text-xs text-slate-500">{item._count.comments} comentários · {item._count.activities} atividades</div>
              </TableCell>
              <TableCell className="text-slate-300">
                <div>{queueLabel(item.queueKey)}</div>
              ...
              <TableCell><TonePill tone={statusTone(item.status)}>{statusLabel(item.status)}</TonePill></TableCell>
              <TableCell><TonePill tone={triageTone(item.triageStatus)}>{triageLabel(item.triageStatus)}</TonePill></TableCell>
              <TableCell className="text-slate-300">
                <div>{item.breachedAt ? <span className="text-rose-200">estourado</span> : <span className="text-emerald-200">no prazo</span>}</div>
                <div className="mt-1 text-xs text-slate-500">{formatDate(item.resolveDueAt)}</div>
              </TableCell>
              <TableCell className="text-slate-300">{item.assignee ? item.assignee.name : "—"}</TableCell>
...
export default async function FilaOperacionalPage({
  ...
  return (
    <AppShell title="Fila operacional" subtitle="Mesa central de triagem, prioridade e despacho do turno.">
      <RegistryHero
        eyebrow="Workbench"
        title="Fila de execução do turno"
        description="A fila continua sendo a mesa principal da operação. A prioridade aqui é reconhecer, atribuir, silenciar com contexto e mandar o caso para a superfície certa."
        actions={
      ...
      <Surface className="p-5">
          <SectionIntro eyebrow="Filtro" title="Busca e recorte" description="O filtro muda a grade principal sem tirar a operação do trilho." compact />
        <form method="GET" className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1.6fr)_repeat(4,minmax(0,170px))_136px]">
          <input name="q" defaultValue={q} placeholder="Buscar por código, título, vínculo ou responsável" className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none" />
          <select name="queueKey" defaultValue={queueKey} className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none">
            <option value="">Todas as filas</option>
            {summary.queues.map((item) => <option key={item.queueKey} value={item.queueKey}>{queueLabel(item.queueKey)}</option>)}
          </select>
          <select name="severity" defaultValue={severity} className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none">
          ...
          <select name="status" defaultValue={status} className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none">
          ...
          <select name="triageStatus" defaultValue={triageStatus} className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none">
          ...
          <button className="rounded-[12px] bg-white px-4 py-3 text-sm font-medium text-black">Aplicar</button>
        </form>
      </Surface>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="grid gap-5">
          {isAdmin ? (
            <Surface className="p-5">
              <SectionIntro eyebrow="Despacho" title="Ação em lote na própria mesa" description="Selecione linhas quando precisar reconhecer, atribuir, silenciar ou resolver sem sair da fila." compact />
              <ActionForm action={bulkApply} className="mt-4 grid gap-4" submitLabel="Aplicar em selecionados" pendingLabel="Aplicando...">
                <div className="grid gap-3 xl:grid-cols-[220px_220px_minmax(0,1fr)]">
                  ...
                  <select name="assigneeUserId" defaultValue="" className="rounded-[12px] border border-white/10 bg-[#0b0f14] px-4 py-3 text-white outline-none">
                  ...
                  <div className="flex items-center text-sm text-slate-400">A leitura principal continua na grade. Essa barra só encurta o despacho quando ele já está claro.</div>
                </div>
          ...
          {!isAdmin && response.items.length ? (
            <Surface className="p-5">
              <SectionIntro eyebrow="Grade" title="Casos da fila" description="Leitura direta da fila para triagem, dono e prazo." compact />
              <div className="mt-4">
                <FilaTable items={response.items} admin={false} />
              </div>
            </Surface>
          ) : null}
