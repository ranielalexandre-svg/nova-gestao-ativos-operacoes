...
async function readActivities() {
  return safeApiJson<PaginatedResponse<ActivityRow>>(
    `/activities${buildApiQuery({ pageSize: 30, sortBy: "createdAt", sortDir: "desc" })}`,
    emptyActivityPage,
  );
}
...
export async function GET() {
  try {
    const [
      ...
      readExceptionQueue({ onlyBreached: "true", pageSize: 30, sortBy: "priorityScore", sortDir: "desc" }),
      readExceptionQueue({ onlyDueSoon: "true", pageSize: 30, sortBy: "priorityScore", sortDir: "desc" }),
      readExceptionQueue({ triageStatus: "pending", status: "open", pageSize: 30, sortBy: "priorityScore", sortDir: "desc" }),
      readExceptionQueue({ onlyUnassigned: "true", pageSize: 30, sortBy: "priorityScore", sortDir: "desc" }),
      readActivities(),
    ]);
    ...
    const pendingActions = [
      exceptionSummary.counts.breachedCount ? `${exceptionSummary.counts.breachedCount} SLA estourado(s)` : "",
      exceptionSummary.counts.dueSoonCount ? `${exceptionSummary.counts.dueSoonCount} caso(s) vencendo` : "",
      exceptionSummary.counts.unassignedCount ? `${exceptionSummary.counts.unassignedCount} sem responsável` : "",
      exceptionSummary.counts.pendingTriageCount ? `${exceptionSummary.counts.pendingTriageCount} em triagem` : "",
      automationSummary.counts.failedRuns24h ? `${automationSummary.counts.failedRuns24h} automação(ões) com falha` : "",
      nocRisk ? `${nocRisk} sinal(is) NOC em atenção` : "",
      reconciliationPending ? `${reconciliationPending} divergência(s) de reconciliação` : "",
    ].filter(Boolean);

    const blocks = [
      ["Pendências", exceptionSummary.counts.openCount, `${exceptionSummary.counts.criticalCount} crítica(s), ${exceptionSummary.counts.breachedCount} SLA estourado(s)`, "/operacao/pendencias"],
      ["Handoff", cases.length, "casos priorizados para passagem de turno", "/operacao/handoff"],
      ["War Room", operationalRisk, "pontuação ponderada de risco operacional", "/operacao/war-room"],
      ["Playbooks", pendingActions.length, "roteiros recomendados para o próximo responsável", "/operacao/playbooks"],
      ["NOC", nocRisk, `${telemetry.counts.down} offline, ${sourceFailures.length} fonte(s) falhando`, "/monitoramento/sensores"],
      ["Automações", automationSummary.counts.failedRuns24h, `${automationSummary.counts.dueRules} regra(s) vencida(s)`, "/operacao/automacoes"],
      ["Reconciliação", reconciliationPending, `${reconciliation.counts.weakUnitMatches} match fraco, ${reconciliation.counts.unmatchedImportedUnits} importado(s) sem par`, "/operacao/reconciliacao"],
      ["Rastro", activities.meta.total, activities.items[0] ? `último ${formatExportDate(activities.items[0].createdAt)}` : "sem evento recente", "/operacao/atividade"],
    ];

    const executive = [
      ["Resumo para gestor", `${reportHealthLabel(reportScore)} com score ${reportScore} e risco operacional ${operationalRisk}.`, "/operacao/war-room"],
      ["Risco residual", pendingActions.length ? `Permanecem ${pendingActions.join("; ")}.` : "Sem risco residual crítico no recorte atual.", "/operacao/pendencias"],
      ["Ações executadas", `${activities.items.length} evento(s) ou decisão(ões) recentes no rastro operacional.`, "/operacao/atividade"],
      ["Pós-incidente", topCase ? `Usar ${topCase.code} como caso principal para revisão, evidências e causa provável.` : "Sem caso principal crítico para pós-incidente.", topCase ? `/operacao/excecoes/${topCase.id}` : "/operacao/relatorio-turno"],
    ];

    const csvRows = [
      ["Seção", "Indicador", "Valor", "Detalhe", "Link"],
      ["Resumo", "Score do turno", reportScore, reportHealthLabel(reportScore), ""],
      ...
      [],
      ["Executivo", "Bloco", "Texto", "Detalhe", "Link"],
      ...executive.map((item) => ["Executivo", item[0], item[1], "", item[2]]),
      [],
      ["Frentes", "Frente", "Valor", "Detalhe", "Link"],
      ...blocks.map((item) => ["Frente", ...item]),
      [],
      ["Ações pendentes", "Item", "Valor", "Detalhe", "Link"],
      ...pendingActions.map((item) => ["Pendente", item, "", "", "/operacao/playbooks"]),
      [],
    ...
    return new Response(`\uFEFF${csv}\n`, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="nova-relatorio-executivo-turno-${filenameDate}.csv"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Não foi possível exportar o relatório executivo do turno." },
      { status: 500 },
    );
  }
}
