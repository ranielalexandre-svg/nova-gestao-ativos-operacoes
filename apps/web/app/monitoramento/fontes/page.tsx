...
function IntegrationHealthStrip({ checks }: { checks: IntegrationCheck[] }) {
  if (!checks.length) {
    return (
      <Panel eyebrow="Saude" title="Fontes ativas">
        <EmptyBlock
...
export default async function IntegracoesPage({
  ...
  if (!session.authenticated) {
    redirect("/login?next=/monitoramento/fontes");
  }
  ...
  if (!isAdmin) {
    return (
      <NovaLitShell activeHref="/monitoramento/fontes">
        <main className="nova-integracoes-page">
          <header className="nova-integracoes-hero">
            <div>
              <span>Monitoramento / Fontes NOC</span>
              <h1>Fontes NOC e integrações</h1>
              <p>Leitura das fontes NOC e contrato de vínculo com o Zabbix.</p>
            </div>
            <div className="nova-integracoes-hero-actions">
              <Link href={hrefWithParams("/monitoramento/fontes", params, {})} className="nova-lit-button nova-lit-button-secondary">Atualizar dados</Link>
          ...
          <section className="nova-integracoes-kpis">
            <StatCard label="Fontes" value={summary.counts.integrationsTotal} hint={`${summary.counts.integrationsActive} ativos`} tone={summary.counts.integrationsActive ? "blue" : "slate"} />
            ...
            <StatCard label="Saude" value={`${healthPercent}%`} hint="fontes ativas OK" tone={summary.counts.integrationsFailing ? "orange" : "green"} />
          </section>
  ...
  const clearFocusHref = hrefWithParams("/monitoramento/fontes", params, { focus: undefined });
      ...
      <main className="nova-integracoes-page">
        <header className="nova-integracoes-hero">
          <div>
            <span>Monitoramento / Fontes NOC</span>
            <h1>Fontes NOC e integrações</h1>
            <p>Fontes, endpoints, credenciais e sincronização segura que alimentam sensores, NOC e relatórios operacionais.</p>
          </div>
          <div className="nova-integracoes-hero-actions">
            <Link href={hrefWithParams("/monitoramento/fontes", params, {})} className="nova-lit-button nova-lit-button-secondary">Atualizar dados</Link>
            <Link href="/monitoramento/sensores" className="nova-lit-button nova-lit-button-secondary">Ver sensores</Link>
            <Link href={hrefWithParams("/monitoramento/fontes", params, { focus: "new" })} className="nova-lit-button nova-lit-button-primary">Nova fonte</Link>
          </div>
        </header>
        ...
        <div className="nova-integracoes-layout">
          <div className="nova-integracoes-main">
            <Panel
              ...
              action={<Link href="/monitoramento/fontes" className="nova-lit-button nova-lit-button-secondary">Limpar</Link>}
            >
              ...
              <div className="nova-integracoes-pagination">
                ...
                <div>
                  <Link
                    ...
                    href={page <= 1 ? "#" : hrefWithParams("/monitoramento/fontes", params, { page: page - 1 })}
                  >
                    Anterior
                  </Link>
                  <Link
                    ...
                    href={page >= totalPages ? "#" : hrefWithParams("/monitoramento/fontes", params, { page: page + 1 })}
                  >
                    Proxima
                  </Link>
                </div>
